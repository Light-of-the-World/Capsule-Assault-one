...
public class TitleScreenTurreT : MonoBehaviour
{
    ...
    public GameObject rangeCollider;             // Sphere around the attack range to check for enemies
    ...
    private void Start()
    {
        enemy = GameObject.FindGameObjectWithTag("TitleEnemy");
        Invoke("FindASuitableTarget", 0.1f);
    ...
    public void TitleColliderTarget(GameObject enemyToTarget)
    {
        Debug.Log("Swapping to target");
        enemy = enemyToTarget;
        Quaternion targetCameraRotation = Quaternion.LookRotation(flatDirection, cameraUpDirection);
        turretCam.transform.rotation = Quaternion.RotateTowards(
        turretCam.transform.rotation,
    ...
    public void FindASuitableTarget()
    {
        // Step 1: Find all enemies
        allEnemies = GameObject.FindGameObjectsWithTag("TitleEnemy");
            ...
            {
                allEnemies = GameObject.FindGameObjectsWithTag("TitleEnemy");
                int failsafeIndex = UnityEngine.Random.Range(0, allEnemies.Length);
                if (failsafeIndex > 0)
                {
                    enemy = allEnemies[failsafeIndex];
                    Quaternion targetCameraRotation = Quaternion.LookRotation(flatDirection, cameraUpDirection);
                    turretCam.transform.rotation = Quaternion.RotateTowards(
                    turretCam.transform.rotation,
                    targetCameraRotation,
                    turretCameraRotationSpeed * Time.deltaTime);
                    Debug.Log("Enemies alive but not in range, retargetting soon");
                    Invoke("FindingANewTarget", 0.1f);
                    return;
                }
                else
                {
                    Debug.LogWarning("NO ENEMIES ALIVE");
                    Invoke("FindingANewTarget", 0.1f);
                    return;
                }
            }
    ...
    private void FindingANewTarget()
    {
        ...
        foreach (GameObject enemy in allEnemies)
        {
            ...
            if (Vector3.Distance(transform.position, enemy.transform.position) <= (attackRange / 2))
            {
        ...
        if (!enemy)
        {
            Debug.Log("No enemy detected, trying again");
            Invoke("FindingANewTarget", 0.02f);
            return;
        }
        Quaternion targetCameraRotation = Quaternion.LookRotation(flatDirection, cameraUpDirection);
        turretCam.transform.rotation = Quaternion.RotateTowards(
    ...
    private void EnsureEnemyIsVisible()
    {
        ...
        if (flatDirection != Vector3.zero)
        {
            ...
            if (Physics.Raycast(ray, out hit)) //hit an enemy or wall
            {
                if (!enemy)
                {
                    Debug.Log("Current enemy is null. Retargetting");
                    FindingANewTarget();
                    return;
                }
                if (hit.collider.CompareTag("Walls"))
                ...
                if (distanceToEnemy >= attackRange)
                {
                    Debug.Log("Current Enemy out of range. Retargetting");
                    Invoke("FindingANewTarget", 0.04f);
                    return;
                }
                /*
